...
$categoryQuery = "SELECT * FROM dress";
$categoriesResult = mysqli_query($dbcon, $categoryQuery);
$categories = [];
if ($categoriesResult) {
...
$categoryFilter = "";
if (isset($_POST['filter']) && !empty($_POST['categoryFilter'])) {
    $categoryFilter = mysqli_real_escape_string($dbcon, trim($_POST['categoryFilter']));
}
...
if (!empty($searchQuery)) {
    $conditions[] = "LOWER(NAME) LIKE LOWER('%$searchQuery%')";
}
...
<div class="category-filter" style="text-align: center; margin-bottom: 20px;">
    <form method="POST" action="">
        <select name="categoryFilter">
            ...
            <?php
                $categories=["TRADITIONAL","WESTERN","PARTY-WEAR","CASUAL"];
        ...
        <div class="featured-dresses">
            ...
            if (!empty($searchQuery)) {
                ...
                $sql .= " WHERE LOWER(NAME) LIKE LOWER('%$searchQuery%')";
            }

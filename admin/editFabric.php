...
if (!empty($_FILES['image-upload']['name'])) {
    ...
    // Update fabric details in the database, including image URL if a new image is uploaded
...
<body>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="text" id="fabric-name" name="fabric-name" value="<?php echo $fabric['NAME']; ?>" required>

        <label for="description">Description:</label>
        <textarea id="description" name="description" required><?php echo $fabric['DESCRIPTION']; ?></textarea>

        <label for="price">Price per Unit:</label>
        <input type="number" id="price" name="price" value="<?php echo $fabric['PRICE_PER_UNIT']; ?>" required>

        <label for="quantity">Available Quantity:</label>
        <input type="number" id="quantity" name="quantity" value="<?php echo $fabric['AVAILABLE_QUANTITY']; ?>" required>
        ...
        <div>
            <p>Current Image:</p>
            <img src="<?php echo $fabric['IMAGE_URL']; ?>" alt="Current Fabric Image" width="100">
        </div>
